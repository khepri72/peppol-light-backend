...
export const registerUploadedInvoice = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { fileName, fileUrl, status, conformityScore, errorsList, errorsData } = req.body;
    ...
    // Try to save with errorsData first (for i18n), fallback to errorsList only if field doesn't exist
    let records;
    try {
      ...
      // If 'Errors Data' field doesn't exist in Airtable, retry without it
      if (error.statusCode === 422 || error.message?.includes('INVALID_FIELD_NAME') || error.message?.includes('Errors Data')) {
        console.log('⚠️  Airtable field "Errors Data" not found, saving without it (legacy mode)');
        records = await base(TABLES.INVOICES).create([
          {
            fields: {
              'User': [userId],
              ...
              'Conformity Score': conformityScore || 0,
              'Errors List': errorsList || '',
              // 'Errors Data' omitted - field doesn't exist yet
            },
          },
        ]);
      } else {
